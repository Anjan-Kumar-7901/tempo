...
export default function ProfilePanel({ onClose }) {
  ...
  return <div className="profile-backdrop" onMouseDown={onClose}>
    <aside className="profile-panel" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <section className="card settings-list"><button onClick={() => { onClose(); navigate('/safe-view') }}><span><strong>Doctor safe view</strong><small>Read-only summary with notes hidden</small></span><b>›</b></button><button onClick={() => { onClose(); navigate('/library') }}><span><strong>Education library</strong><small>Offline cycle articles</small></span><b>›</b></button><button onClick={() => { setMessage(''); setPinOpen(true) }}><span><strong>Change PIN</strong><small>Update your local privacy lock</small></span><b>›</b></button><button onClick={() => { lock(); navigate('/unlock') }}><span><strong>Lock now</strong><small>TEMPO also locks after 5 minutes inactive</small></span><b>›</b></button><button className="danger-text" onClick={() => setResetOpen(true)}><span><strong>Reset all data</strong><small>Permanently erase this device’s TEMPO data</small></span><b>›</b></button></section>
